...
def is_context_only_followup(text: str) -> bool:
    ...
    cue = re.compile(
        r"(どういうこと(?:なの)?(?:か)?|どういう意味(?:なの)?(?:か)?|何を意味(?:する)?(?:の)?(?:か)?|つまり|要するに|簡単に|かみ砕|もう一度|もう少し|"
    ...
    residual = re.sub(
        ...
        r"です|ます|でした|ました|は|が|も|を|の|に|で|か)+",
        " ",
        residual,
        flags=re.I,
    )
...
class ReflectiveConversationOrgan:
    ...
    CONTEXTUAL_FOLLOWUP = re.compile(
        r"(どういうこと(?:なの)?(?:か)?|どういう意味(?:なの)?(?:か)?|何を意味(?:する)?(?:の)?(?:か)?|つまり|要するに|"
        r"簡単に|かみ砕|もう一度|もう少し|"
        r"(?:わか|分か|理解)(?:ります|できます|できる).*(?:か|？|\?))",
        re.I,
    )
    ...
    def _question_mode(text: str) -> str:
        ...
        if re.search(r"(なぜ|どうして|理由|原因|why)", t, re.I):
            return "why"
        if re.search(r"(どういうこと|どういう意味|何を意味|つまり|要するに|簡単に|かみ砕)", t, re.I):
            return "clarify"

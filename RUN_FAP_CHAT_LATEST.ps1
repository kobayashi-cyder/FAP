...
$Gateway = Join-Path $Root 'fap_v1_0_01_dynamic_sparse_routing_gateway.py'
...
try {
    ...
    & $Python.Exe @Prefix '-c' "import fap_v1_0_01_dynamic_sparse_routing_gateway as g; s=g.CORE.chat_status(); print('[OK] preflight version=' + str(s.get('version',''))); raise SystemExit(0 if s.get('version') == '1.0.01-unified-chat' else 4)"
    if ($LASTEXITCODE -ne 0) {

...
$LatestVersion = '87.43-unified-chat'
$Gateway = Join-Path $Root 'fap_v87_43_reflective_chat_gateway.py'
...
try {
    ...
    & $Python.Exe @Prefix '-c' "import fap_v87_43_reflective_chat_gateway as g; s=g.CORE.chat_status(); print('[OK] preflight version=' + str(s.get('version',''))); raise SystemExit(0 if s.get('version') == '87.43-unified-chat' else 4)"
    if ($LASTEXITCODE -ne 0) {

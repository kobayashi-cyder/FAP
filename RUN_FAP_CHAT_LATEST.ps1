...
$LatestVersion = '87.39-unified-chat'
$Gateway = Join-Path $Root 'fap_v87_39_native_geometry_gateway.py'
...
try {
    ...
    & $Python.Exe @Prefix '-c' "import fap_v87_39_native_geometry_gateway as g; s=g.CORE.status(); print('[OK] preflight version=' + str(s.get('version',''))); raise SystemExit(0 if s.get('version') == '87.39-unified-chat' else 4)"
    if ($LASTEXITCODE -ne 0) {

...
from fap_revision_r001 import (
    Signal as AdaptiveSignal,
    adaptive_budget as r001_adaptive_budget,
)
...
class InteractionFabric:
        ...
        *,
        ...
        if len(ranked) >= 2:
            ...
            if sparse_router is not None:
                ...
                r001_budget = self._r001_route_budget(
                    sparse_context,
                    request.metadata,
                )
                sparse_decision = sparse_router.select(
                    sparse_context,
                    top_k=min(
                        budget.route_candidates,
                        max(1, int(r001_budget.routes)),
                        len(ranked),
                    ),
                    max_path_routes=max(1, int(r001_budget.routes)),
                )
                row_by_id = {
    ...
    @staticmethod
    def _r001_route_budget(
        context: RoutingContext,
        metadata: Mapping[str, Any] | None,
    ):
        raw_counts = (
            metadata.get("failure_counts", {})
            if isinstance(metadata, Mapping)
            else {}
        )
        counts = raw_counts if isinstance(raw_counts, Mapping) else {}
        signals = []
        for failure in context.failure_classes[:16]:
            try:
                count = int(counts.get(failure, 1))
            except (TypeError, ValueError, OverflowError):
                count = 1
            signals.append(
                AdaptiveSignal(
        ...
        return r001_adaptive_budget(
            signals,
            uncertainty=context.uncertainty,
            disagreement=context.verifier_disagreement >= 0.5,
        )
        ...
        *,
        ...
        return {
            ...
            "adaptive_route_budget": {
                "routes": int(r001_budget.routes),
                "steps": int(r001_budget.steps),
                "verify": int(r001_budget.verify),
                "retries": int(r001_budget.retries),
            },
        }

...
public final class FapEngine {
    ...
    public Result process(String q){
        ...
        if(lower.contains("fap") && (lower.contains("とは") || lower.contains("what"))){
            return new Result(q,"FAP V59 Androidは、必要なSkillだけを選び、検証済みの成功/失敗からSkill Graphを局所更新する軽量ローカル実装です。未検証の会話は恒久学習に使いません。","knowledge_local",.90);
        }
        if(q.matches(".*(こんにちは|こんばんは|おはよう|hello|hi).*")){
            return new Result(q,"こんにちは。FAP V59 Androidでローカル処理しています。","dialogue",.90);
        }
    ...
    public String status(){return "READY · V59 · turns="+turns+" · learnedEdges="+graph.edgeCount();}

...
_HISTORY_RECALL = re.compile(
    r"(前(?:の|回|章|に)|以前|過去|これまで|さっき|先ほど|覚えて|思い出|振り返|"
    r"続き|続け|引き続き|前提を引き継|前の話)"
)
_NEW_CHAPTER = re.compile(
    r"(新章|別件|話題を変|話を変|次の話題|新しい話題|ここから別|ところで|さて[,、 ]?)"
)
...
class AdaptiveContextSelection:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "history_turn_cap": self.history_turn_cap,
            "chapter_boundary": self.chapter_boundary,
        }
...
class AdaptiveContextSelector:
    ...
    def select(
        ...
        *,
        ...
        # Chapter-aware context policy: recent context is the default. Older
        # history is reopened only when the user explicitly asks to recall or
        # continue prior material. An explicit topic/chapter change tightens
        # the window further so old subjects do not leak into the new one.
        query = str(text or "")
        recall_history = bool(_HISTORY_RECALL.search(query))
        chapter_boundary = bool(_NEW_CHAPTER.search(query))
        if recall_history:
            context_mode = "historical_recall"
            history_turn_cap = turn_limit
        elif chapter_boundary:
            context_mode = "new_chapter_recent"
            history_turn_cap = min(turn_limit, 4)
        else:
            context_mode = "recent"
            history_turn_cap = min(turn_limit, 8)
        turn_limit = max(1, history_turn_cap)

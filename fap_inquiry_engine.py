...
class InquiryEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self.persistent_learning = bool(persistent_learning)
        self.max_target = max(16, min(2048, int(max_target)))
        configured_target = _env_int("FAP_INQUIRY_TARGET", 256, 32, 2048)
        self.default_target = min(
            self.max_target,
            max(1, int(default_target)) if default_target is not None else configured_target,
        )
        configured_rounds = _env_int("FAP_INQUIRY_ROUNDS", 8, 1, 16)
        self.max_rounds = max(
            1,
            min(16, int(max_rounds)) if max_rounds is not None else configured_rounds,
        )
    ...
    def _target_from_text(self, text: str) -> int:
        ...
        if m:
            return max(1, min(self.max_target, int(m.group(1))))
        if re.search(r"(とにかく増|大量|できるだけ多|最大限|徹底的|限界まで|可能な限り)", str(text or "")):
            return self.max_target
        return min(self.default_target, self.max_target)
    ...
    def _answer_question(
        ...
        recalled = (
            self.ledger.recall(question.question)
            if self.persistent_learning
            else None
        )
    ...
    def _inject_frontier(self, topic: str, questions: list[InquiryQuestion], target: int) -> int:
        if not self.persistent_learning:
            return 0
        if len(questions) >= target:
            return 0
    ...
    def run(self, text: str, history: list[Mapping]) -> dict | None:
        ...
        return {
            ...
            "persistent_epistemic_learning": self.persistent_learning,
            "contradiction_quarantine": self.persistent_learning,

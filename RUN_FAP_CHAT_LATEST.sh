...
LATEST_VERSION="87.76-unified-chat"
GATEWAY="$ROOT/fap_v87_76_session_continuity_gateway.py"
...
gateway_preflight() {
  PYTHONNOUSERSITE=1 PYTHONDONTWRITEBYTECODE=1 "$PY" -S -B -c     'import fap_v87_64_raster_quality_gateway as g; s=g.CORE.chat_status(); raise SystemExit(0 if s.get("version") == "87.76-unified-chat" else 4)'     >/dev/null 2>&1
}

...
GATEWAY="$ROOT/fap_v1_0_01_dynamic_sparse_routing_gateway.py"
...
gateway_preflight() {
  PYTHONNOUSERSITE=1 PYTHONDONTWRITEBYTECODE=1 "$PY" -S -B -c     'import fap_v1_0_01_dynamic_sparse_routing_gateway as g; s=g.CORE.chat_status(); raise SystemExit(0 if s.get("version") == "1.0.01-unified-chat" else 4)'     >/dev/null 2>&1
}

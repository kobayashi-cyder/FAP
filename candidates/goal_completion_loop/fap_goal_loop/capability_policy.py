...
class CapabilityRegistration:
    """Capability registration with a fail-closed local-retry safety contract."""
    ...
    def effective_handler(self) -> CapabilityHandler:
        if not self.retry_safe or self.max_attempts == 1:
            return self.handler
        return ResilientCapabilityHandler(self.handler, policy=RetryPolicy(max_attempts=self.max_attempts), retryable_errors=self.retryable_errors)


def build_capability_handlers(registrations: Iterable[CapabilityRegistration]) -> Mapping[str, CapabilityHandler]:

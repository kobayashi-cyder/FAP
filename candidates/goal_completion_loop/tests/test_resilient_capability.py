...
class ResilientCapabilityTests(unittest.TestCase):
    def test_retry_policy_is_fail_closed(self):
        ...
        with self.assertRaises(ValueError):
            CapabilityRetryPolicy(2, True, "read only", max_total_retry_delay_seconds=241)
        with self.assertRaises(ValueError):
            CapabilityRetryPolicy(2, True, "read only", retry_delay_seconds=1, max_total_retry_delay_seconds=0)
    ...
    def test_total_retry_delay_budget_caps_long_backoff(self):
        calls, delays = [], []
        def handler(instruction, metadata, goal_state):
            calls.append(1)
            return ExecutionResult("failed", error="busy", metadata={"transient": True})
        executor = ResilientCapabilityExecutor({"read": handler}, retry_policies={"read": CapabilityRetryPolicy(5, True, "read only", retry_delay_seconds=10, retry_backoff_multiplier=2, max_total_retry_delay_seconds=45)}, sleeper=delays.append)
        result = executor.execute(action(), state())
        self.assertEqual(result.status, "failed")
        self.assertEqual(result.metadata["attempts"], 5)
        self.assertEqual(delays, [10.0, 20.0, 15.0])
        self.assertEqual(result.metadata["retry_delay_seconds"], 45.0)

...
class CapabilityPolicyTests(unittest.TestCase):
    ...
    def test_unsafe_handler_is_never_wrapped_or_retried(self):
        calls = {"n": 0}
        ...
        handlers = build_capability_handlers([CapabilityRegistration("write", handler)])
        result = handlers["write"]("x", {}, _state())
        self.assertEqual(result.status, "failed")
        self.assertEqual(calls["n"], 1)

    def test_retry_safe_handler_retries_transient_failure(self):
        calls = {"n": 0}
        ...
        handlers = build_capability_handlers([
            CapabilityRegistration(
                ...
                handler,
                retry_safe=True,
                idempotency_basis="read-only request with no external mutation",
                max_attempts=2,
            )
        ])

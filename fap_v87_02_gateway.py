...
class IntentOrgan:
    def classify(self, text: str) -> Intent:
        ...
        # Specific task words beat generic temporal modifiers such as "今日".
...
class FAPV8702:
    ...
    def capabilities(self) -> list[str]:
        caps = ["intent", "datetime", "calculator", "memory", "verification", "fap-eval"]
        caps.append("weather")
        caps.append("image-routing")
        if self.image.available()[0]:
            caps.append("image-generation")
        if self.e2b.available():
            caps.append("gemma4:e2b")
        return caps

    def status(self) -> dict:
        e2b = self.e2b.available()
        return {
            "name": "FAP",
            "version": VERSION,
            "state": "ready" if e2b else "degraded",
            "model": OLLAMA_MODEL,
            "e2b": e2b,
    ...
    def route(self, intent: Intent, text: str, history: list[dict]) -> dict:
        ...
        return self.e2b.run(text, history)
    ...
    def do_GET(self):
        ...
        if path.startswith("/artifacts/"):
            ...
            if not file.exists() or file.parent != ARTIFACTS:
                ...
                return
            raw = file.read_bytes()
            ctype = "image/png" if file.suffix.lower() == ".png" else "application/octet-stream"
            self.send_response(200)
...
def main() -> None:
    ...
    print(f"FAP V{VERSION} Capability Gateway")

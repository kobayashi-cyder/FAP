...
from fap_builder import ArtifactBuilder, BuilderError

ROOT = Path(__file__).resolve().parent
WEB_FILE = ROOT / "web" / "FAP_Chat.html"
RUNTIME = ROOT / "runtime" / "v87_04"
...
class IntentOrgan:
    def classify(self, text: str) -> Intent:
        ...
        # Generic local Builder. Image creation remains owned by ImageOrgan.
        build_verb = bool(re.search(r"(作って|作成|生成|構築|build|create|make)", t, re.I))
        build_target = bool(re.search(r"(テトリス|tetris|html|web|ウェブ|ゲーム|game|python|json|設定ファイル|スクリプト|script|ファイル|tool|ツール)", t, re.I))
        if build_verb and build_target and not image_word:
            scores["builder"] += 1.08
...
class FAPV8704:
    def __init__(self):
        ...
        self.builder = ArtifactBuilder(ARTIFACTS, RUNTIME / "workspace")
        self.e2b = E2BOrgan()
    ...
    def capabilities(self) -> list[str]:
        caps = [
            ...
            "builder-stage1", "builder:single-html", "builder:tetris", "builder:json",
    ...
    def status(self) -> dict:
        ...
        return {
            ...
            "builder": {"stage": 1, "local": True, "recipes": ["tetris", "generic_page", "generic_config"]},
            "e2b": teacher,
    ...
    def route(self, intent: Intent, text: str, history: list[dict]) -> dict:
        ...
        if intent.name == "builder":
            try:
                ...
                return {"ok": False, "reply": f"Builder Stage 1では完了できませんでした: {e}", "confidence": 0.86}
...
def main() -> None:
    ...
    print(f"FAP V{VERSION} Distilled + Builder Capability Gateway")
    print(f"UI: http://{HOST}:{PORT}/")
    print("Local brain: V78 distilled circuits + teacher-shadow")
    print("Builder: V87.04 constrained local artifact builder (Stage 1)")

...
class DistilledFAPOrgan:
    ...
    def _primitive_reply(self, text: str, history: list[Mapping]) -> str | None:
        ...
        latency = re.search(r"(?:(\d+(?:\.\d+)?)\s*秒|応答|返答|回答).{0,24}(?:かか|遅|待)|(?:かか|遅|待).{0,24}(?:応答|返答|回答|\d+(?:\.\d+)?\s*秒)", t)
        if latency:
            sec = re.search(r"(\d+(?:\.\d+)?)\s*秒", t)
            observed = (sec.group(1) + "秒程度") if sec else "その程度"
            return (
                f"{observed}の応答遅延という実測報告として受け取りました。"
                "ローカル経路としては長すぎます。回答生成だけでなく、状態集計や外部可用性確認などの周辺処理も切り分け対象です。"
            )

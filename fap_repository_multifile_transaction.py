...
class RepositoryTransactionBuilder:
    """Validate and fingerprint coherent multi-file FileEdit transactions."""
    ...
    def build(
        ...
        *,
        ...
        if require_all_mutations:
            ...
            if missing:
                raise ValueError(
                    "transaction missing planned mutations: " + ",".join(missing)
                )

        canonical = tuple(sorted(members, key=lambda x: x.path))
        raw = json.dumps(
            [asdict(row) for row in canonical],
            sort_keys=True,
            separators=(",", ":"),
        )
        digest = sha256(
        ...
        return RepositoryEditTransaction(
    ...
    @staticmethod
    def equivalent(
        left: RepositoryEditTransaction,
        right: RepositoryEditTransaction,
    ) -> bool:
        return (

...
if /I "%EXISTING%"=="87.39-unified-chat" (
...
powershell.exe -NoProfile -Command ^
  "$u='http://127.0.0.1:%PORT%/api/v1/status'; $d=(Get-Date).AddSeconds(15); while((Get-Date) -lt $d){ try { $s=Invoke-RestMethod -TimeoutSec 1 $u; if($s.version -eq '87.39-unified-chat'){ exit 0 } } catch {}; Start-Sleep -Milliseconds 75 }; exit 1"

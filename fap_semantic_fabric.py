...
class SemanticFabricBridge:
    ...
    def bind(
        ...
        return InteractionEndpoint(
            ...
            cost=endpoint.cost,
            capabilities=tuple(
                dict.fromkeys(endpoint.capabilities + routes)
            ),
            task_families=endpoint.task_families,
            task_forms=endpoint.task_forms,
            failure_specialties=endpoint.failure_specialties,
        )

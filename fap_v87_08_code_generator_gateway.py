...
from fap_code_generator import CodeGeneratorOrgan, CodeGenerationError

ROOT = Path(__file__).resolve().parent
WEB_FILE = ROOT / "web" / "FAP_Chat.html"
RUNTIME = ROOT / "runtime" / "v87_08"
...
def http_json(url: str, method: str = "GET", data: dict | None = None, timeout: float = 12.0) -> Any:
    ...
    req.add_header("User-Agent", "FAP-V87.08/1.0")
...
class FAPV8708:
    def __init__(self):
        ...
        self.codegen = CodeGeneratorOrgan(ARTIFACTS, RUNTIME / "code_workspace")
        self.e2b = E2BOrgan()
    ...
    def capabilities(self) -> list[str]:
        caps = [
            ...
            "builder-stage4", "builder:spec-compiler", "code-generator-stage1", "codegen:python", "codegen:html", "codegen:json", "codegen:markdown", "codegen:self-test", "codegen:repair-loop", "builder:constraint-extractor", "builder:generic-grid-composer", "builder:concept-resolver", "builder:mechanism-decomposition", "builder:generic-intent", "builder:composition", "builder:single-html", "builder:tetris", "builder:breakout", "builder:pong", "builder:snake", "builder:minesweeper", "builder:2048", "builder:memory-match", "builder:json",
    ...
    def status(self) -> dict:
        ...
        return {
            ...
            "code_generator": {"stage": 1, "local": True, "mode": "codespec-generate-verify-selftest-repair", "targets": ["python_calculator", "python_csv_stats", "html_notepad", "html_counter", "json_config", "markdown_document"]},
            "e2b": teacher,
        ...
        if intent.name == "builder":
            if self.codegen.claims(text):
                try:
                    ...
                    return {"ok": False, "reply": f"Code Generator Stage 1では完了できませんでした: {e}", "confidence": 0.86}
            try:
...
def main() -> None:
    ...
    print(f"FAP V{VERSION} Distilled + Specification Compiler + Code Generator Gateway")
    ...
    print("Code Generator: V87.08 CodeSpec + self-test + repair loop (Stage 1)")

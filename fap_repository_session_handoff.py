...
class RepositorySessionHandoffCodec:
    """Content-free, tamper-evident handoff between coding chat sessions."""
    ...
    def decode(self, token: str) -> RepositorySessionHandoff:
        ...
        supplied = str(row.get("checksum") or "")
        payload = {
            "contract": self.CONTRACT,
            ...
            "plan_id": _hex(row.get("plan_id"), "plan_id", minimum=64, allow_empty=False),
            "repository_digest": _hex(
            ...
            "paths": tuple(_path(x) for x in row.get("paths", ())),

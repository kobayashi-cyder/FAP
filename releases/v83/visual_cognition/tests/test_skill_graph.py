...
class VisualSkillGraphTests(unittest.TestCase):
    def test_feedback_cycle_is_active(self):
        ...
        self.assertEqual(
            ...
            {"imagine", "draw", "see", "review", "repair", "motion"},
        )
        edges = {(edge["source"], edge["target"]) for edge in manifest["edges"]}
        self.assertTrue({
            ("imagine", "draw"),
            ("draw", "see"),
            ("see", "review"),
    ...
    def test_graph_executes_verified_visual_cycle(self):
        graph = VisualSkillGraph()
        result = graph.execute_text(
            "24x24のキャンバス; 青い四角 x=3 y=4 w=6 h=5"
        )

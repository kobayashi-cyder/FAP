...
class VisualDiffer:
    """Structured target-vs-Vision comparison at primitive/field granularity."""
    ...
    def compare(self, target: VisualIR, observed: VisualIR) -> tuple[VisualDifference, ...]:
        ...
        for expected in target.primitives:
            ...
            if actual is None:
                diffs.append(
                    VisualDifference(expected.primitive_id, "missing", expected, None, 1.0)
                )
                continue
            unmatched.remove(actual)
            if expected.kind != actual.kind:
                diffs.append(
                    VisualDifference(expected.primitive_id, "kind", expected.kind, actual.kind, 1.0)
                )
            if expected.color != actual.color:
                magnitude = math.sqrt(
                    sum((a - b) ** 2 for a, b in zip(expected.color, actual.color))
                ) / 441.673
                diffs.append(
                    VisualDifference(
                        expected.primitive_id, "color",
                        expected.color, actual.color, round(magnitude, 4),
                    )
                )
            if (expected.x, expected.y) != (actual.x, actual.y):
                magnitude = math.hypot(
                    expected.x - actual.x, expected.y - actual.y
                ) / max(1.0, math.hypot(target.width, target.height))
                diffs.append(
                    VisualDifference(
                        expected.primitive_id, "position",
            ...
            if expected.kind == actual.kind == "circle" and expected.radius != actual.radius:
                diffs.append(
                    VisualDifference(
                        expected.primitive_id, "radius",
                        expected.radius, actual.radius,
                        float(abs(expected.radius - actual.radius)),
                    )
                )
            if (
                expected.kind == actual.kind == "rect"
                and (expected.width, expected.height) != (actual.width, actual.height)
            ):
                diffs.append(
                    VisualDifference(
                        expected.primitive_id, "size",
    ...
    @staticmethod
    def _nearest(
        expected: VisualPrimitive,
        candidates: list[VisualPrimitive],
        ...
        return min(
            pool,
            key=lambda p: (p.center[0] - ex) ** 2 + (p.center[1] - ey) ** 2,
        )


class LocalVisualRepairer:
    """Patch only target fields named by VisualDifference."""

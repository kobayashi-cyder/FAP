...
class VisualSkillGraph:
    """Visual cognition subgraph sharing VisualIR as its central state."""
    ...
    def _validate(self) -> None:
        missing = [
            name for name in self.REQUIRED_NODES
            if name not in self.nodes or not self.nodes[name].active
        ]
        if missing:
        ...
        if not cycle.issubset(edge_pairs):

...
class NaturalLanguageVisualPlanner:
    """Small bootstrap NL -> VisualIR planner.

    It intentionally handles only bounded visual facts (shape/color/position/size).
    Later language planners can replace it without changing VisualIR or the feedback loop.
    """
    ...
    def plan(self, text: str) -> VisualIR:
        ...
        for chunk in chunks:
            ...
            if kind == "circle":
                ...
                primitives.append(
                    VisualPrimitive(
                        f"p{len(primitives)}", "circle", x, y,
                        radius * 2 + 1, radius * 2 + 1, radius, color,
                    )
                )
            else:
    ...
    def _canvas(self, text: str) -> tuple[int, int]:
        m = re.search(r"(?:canvas|キャンバス)\s*=\s*(\d+)\s*[x×]\s*(\d+)", text, re.I)
        if not m:
            m = re.search(r"(\d+)\s*[x×]\s*(\d+)\s*(?:の)?(?:canvas|キャンバス)", text, re.I)
        if not m:

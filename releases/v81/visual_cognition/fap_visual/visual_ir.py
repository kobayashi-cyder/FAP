...
def rgb(r: int, g: int, b: int) -> RGB:
    return (_bounded_channel(r), _bounded_channel(g), _bounded_channel(b))
...
class VisualIR:
    ...
    def sample(self, t: float) -> "VisualIR":
        ...
        return replace(self, primitives=tuple(state[p.primitive_id] for p in self.primitives), motions=())


def with_primitives(width: int, height: int, primitives: Iterable[VisualPrimitive], *, background: RGB = (255,255,255)) -> VisualIR:

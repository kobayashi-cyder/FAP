...
class PrimitiveCircuitBridge:
    """Adopt only evidence-promoted V80 primitives into the V81 sparse circuit pool."""
    ...
    def sync_active(self) -> list[str]:
        """Synchronize V80 SkillRegistry.active() into verifier-approved V81 circuits."""
        adopted: list[str] = []
        for record in self.skill_registry.active():
            ...
            passed = self.controller.certify(
                pid,
                evidence_id=f"v81-primitive-adopt:{pid}:{evidence_digest}",
    ...
    def observe(
        ...
        *,
        ...
        return self.controller.observe(
            decision,
            evidence_id=f"v81-primitive-runtime:{evidence_id}",
            reward=reward,
            success=success,

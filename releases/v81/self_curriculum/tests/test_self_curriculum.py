...
V79_ROOT = ROOT.parents[2] / "v79" / "creativity_engine"
...
from fap_self_curriculum import (
    ...
    build_v81_curriculum,
)


class V81SelfCurriculumTests(unittest.TestCase):
    def test_success_is_compressed_but_failure_is_not(self):
        def solver(task, priors):
            ...
            return AttemptResult(answer, {"sandbox": task.task_id})
        ...
        events = engine.run(6)
        self.assertTrue(any(e["stored_success_pattern"] for e in events))
        self.assertTrue(
            all(
                not e["stored_success_pattern"]
                for e in events
                if e["task"]["ability"] == "generalization"
            )
        )
        self.assertGreater(
            engine.ability_map.snapshot()["composition"]["frontier"],
            0.25,
        )

    def test_non_independent_success_does_not_enter_memory(self):
        task = CurriculumTask("t1", "verification", "verify x", 0.4)
        ...
        with self.assertRaises(ValueError):
            SuccessCompressor().compress(task, attempt, verification)

    def test_generated_task_is_slightly_above_frontier(self):
        state = AbilityMap(["repair"]).states["repair"]
        task = CurriculumGenerator(stretch=0.10).generate(state, [])
        self.assertGreater(task.difficulty, state.frontier)
        self.assertLessEqual(task.difficulty, 0.50)

    def test_focus_does_not_starve_untried_ability(self):
        amap = AbilityMap(["a", "b"])
        fail = VerificationResult(False, 0.0, "fail")
    ...
    def test_pattern_store_persists_only_compressed_record(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            task = CurriculumTask("t2", "repair", "repair parser", 0.5)
            attempt = AttemptResult(
            ...
            pattern = SuccessCompressor().compress(task, attempt, vr)
            self.assertTrue(store.add(pattern))
            payload = json.loads(path.read_text(encoding="utf-8"))
            encoded = json.dumps(payload, ensure_ascii=False)
            self.assertNotIn(attempt.answer, encoded)
            reloaded = PatternStore(path)
            self.assertEqual(len(reloaded.for_ability("repair")), 1)
            self.assertEqual(
                reloaded.for_ability("repair")[0].stage,
                "ephemeral",
            )
    ...
    def test_repeated_independent_success_promotes_pattern(self):
        ...
        for i in range(3):
            ...
            self.assertTrue(
                store.add(compressor.compress(task, attempt, vr))
            )
        patterns = store.for_ability("composition")
        self.assertEqual(len(patterns), 1)
        self.assertEqual(patterns[0].stage, "consolidated")
        self.assertEqual(len(patterns[0].evidence_digests), 3)
    ...
    def test_solver_exception_becomes_learning_failure_not_crash(self):
        ...
        def verifier(task, attempt):
            self.fail("verifier should not run after solver error")
        ...
        self.assertEqual(
            engine.ability_map.snapshot()["repair"]["attempts"],
            1,
        )
    ...
    def test_generic_facade_exposes_capability_map(self):
        ...
        self.assertEqual(
            fap.capability_map()["composition"]["successes"],
            2,
        )
        self.assertTrue(fap.compressed_successes("composition"))

    def test_real_v80_primitive_sandbox_bridge(self):
        with tempfile.TemporaryDirectory() as td:
            fap = build_primitive_self_curriculum(
                pattern_store_path=Path(td) / "patterns.json",
            ...
            self.assertEqual(
                {e["task"]["ability"] for e in events},
                {
            ...
            self.assertEqual(
                sum(x["successes"] for x in fap.capability_map().values()),
                3,
            )
            self.assertGreaterEqual(len(fap.active_primitives()), 3)

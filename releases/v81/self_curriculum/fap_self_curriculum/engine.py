...
class AbilityMap:
    """Tracks what FAP can currently do and where learning value is highest."""

    def __init__(self, abilities: Iterable[str]):
        names = list(dict.fromkeys(str(x).strip() for x in abilities if str(x).strip()))
        if not names:
    ...
    def update(
        ...
        state.attempts += 1
        reward = max(0.0, min(1.0, float(verification.reward)))
        state.ema_reward = 0.72 * state.ema_reward + 0.28 * reward
        if verification.passed:
            state.successes += 1
            state.frontier = max(
                state.frontier,
                min(1.0, 0.65 * state.frontier + 0.35 * float(difficulty)),
            )
        state.uncertainty = max(0.08, 1.0 / (1.0 + 0.35 * state.attempts))
        state.stagnation = (
            0 if state.frontier > old_frontier + 1e-9 else state.stagnation + 1
        )
        return state
...
class PatternStore:
    ...
    def add(self, pattern: SuccessPattern) -> bool:
        fresh = [d for d in pattern.evidence_digests if d not in self.evidence_seen]
        if not fresh:
            return False

        old = self.patterns.get(pattern.pattern_id)
        if old:
            digests = tuple(dict.fromkeys(old.evidence_digests + tuple(fresh)))
            reward = round(
                (
    ...
    def for_ability(self, ability: str, limit: int = 3) -> list[SuccessPattern]:
        rows = [p for p in self.patterns.values() if p.ability == ability]
        rows.sort(key=lambda p: (-p.reward, -p.difficulty, p.pattern_id))
        return rows[: max(0, int(limit))]


class SuccessCompressor:
    """Irreversibly keeps reusable structure, not the raw reasoning trace."""
    ...
    def compress(
        ...
        shared = [x for x in answer_terms if x in task_set][:3]
        distinctive = [x for x in answer_terms if x not in task_set][:5]
        signature = tuple((shared + distinctive)[:8]) or (task.ability,)
        pattern_id = sha256(
            (task.ability + "|" + "|".join(signature)).encode("utf-8")
        ).hexdigest()[:16]

        return SuccessPattern(
            pattern_id=pattern_id,
            ability=task.ability,
            summary=f"{task.ability}: " + " -> ".join(signature[:6]),
            signature=signature,
            difficulty=round(float(task.difficulty), 4),
            evidence_digests=(digest,),
            reward=round(max(0.0, min(1.0, verification.reward)), 4),
        )
...
class CurriculumGenerator:
    ...
    def __init__(
        ...
        *,
        ...
        self.stretch = max(0.02, min(0.25, float(stretch)))
        self.counter = 0
    ...
    def next_difficulty(self, state: AbilityState) -> float:
        return min(
            ...
            max(
                0.10,
                state.frontier
                + self.stretch * (0.75 + state.uncertainty),
            ),
        )
    ...
    def generate(
        ...
        template = self.templates.get(
            state.ability,
            (
                "Solve a new {ability} task at difficulty {difficulty:.2f}; "
                "reuse prior structure only when it generalizes. Prior: {prior}"
            ),
        )
        return CurriculumTask(
            ...
            novelty=round(
                1.0
                if not patterns
                else max(0.25, 1.0 - 0.12 * len(patterns)),
                4,
            ),
        )
...
class SelfCurriculumEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self.ability_map = AbilityMap(abilities)
        self.solver = solver
    ...
    def step(self) -> dict:
        ...
        event = {
            ...
            "pattern_id": pattern_id,
            "ability_after": self.ability_map.snapshot()[task.ability],
        }
        self.history.append(event)
        return event

    def run(self, steps: int = 1) -> list[dict]:
        return [self.step() for _ in range(max(0, int(steps)))]

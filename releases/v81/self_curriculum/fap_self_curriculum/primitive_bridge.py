...
class PrimitiveExerciseGenerator(CurriculumGenerator):
    """Generates bounded tasks that the V80 Mini-IR sandbox can actually verify."""
        ...
        if state.ability == "string_normalization":
            ...
            if composite:
                examples = [
                    ...
                    {"value": f" World{n} ", "expected": f"world{n}"},
                    {"value": "  FAP", "expected": "fap"},
                    {"value": "MiXeD  ", "expected": "mixed"},
                    {"value": "   ", "expected": "", "boundary": True},
                ]
                holdouts = [
                    {"value": f" ALPHA{n} ", "expected": f"alpha{n}"},
            ...
            examples = [
                ...
                {"value": 0, "expected": delta, "boundary": True},
            ...
            if reverse:
                examples = [
                    {"value": [1, 2, 3], "expected": [3, 2, 1]},
                    {"value": ["a", "b"], "expected": ["b", "a"]},
                    {"value": [True, False], "expected": [False, True]},
                    {"value": [1], "expected": [1]},
                    {"value": [], "expected": [], "boundary": True},
                ]
                holdouts = [
                    {"value": [4, 5, 6, 7], "expected": [7, 6, 5, 4]},
                    {"value": ["x", "y", "z"], "expected": ["z", "y", "x"]},
                    {"value": [0, 1], "expected": [1, 0]},
                ]
                target = "reverse"
            else:
                examples = [
                    {"value": [1, 1, 2], "expected": [1, 2]},
                    {"value": ["a", "a"], "expected": ["a"]},
                    {"value": [1, 2, 1], "expected": [1, 2]},
                    {"value": [True, True, False], "expected": [True, False]},
                    {"value": [], "expected": [], "boundary": True},
                ]
                holdouts = [
                    {"value": [3, 3, 4], "expected": [3, 4]},
                    {"value": ["x", "x", "y"], "expected": ["x", "y"]},
            ...
            raise ValueError(f"unsupported primitive curriculum ability: {state.ability}")

        prior = patterns[0].summary if patterns else "none"
        prompt = (
            f"Invent a safe Mini-IR primitive for {state.ability}; "
            f"target={payload['target']}; difficulty={difficulty:.2f}; "
            f"reuse prior only if it generalizes: {prior}"
        )
        return CurriculumTask(
            task_id=f"primitive-self:{state.ability}:{self.counter}",
            ability=state.ability,
            prompt=prompt,
            difficulty=round(difficulty, 4),
            parent_pattern_id=patterns[0].pattern_id if patterns else "",
            novelty=round(1.0 if not patterns else 0.7, 4),
            payload=payload,
        )


class PrimitiveSelfCurriculum:
    """Concrete V81 bridge to V80 PrimitiveInventor -> Sandbox -> Registry."""
    ...
    def __init__(
        ...
        *,
        ...
        self.sandbox = MiniIRSandbox(timeout_ms=timeout_ms)
        ...
        self._candidates = {}
        self.generator = PrimitiveExerciseGenerator(stretch=stretch)
    ...
    def _solve(self, task: CurriculumTask, priors: list[SuccessPattern]):
        examples = tuple(self._case(x) for x in task.payload["examples"])
        candidate = self.inventor.invent_from_examples(
            name=f"self_{task.ability}",
            input_kind=str(task.payload["input_kind"]),
            examples=examples,
            description=task.prompt,
            max_depth=2,
        )
        self._candidates[task.task_id] = candidate
        program = [ins.op for ins in candidate.program]
        return AttemptResult(
            answer=f"{task.ability} " + " -> ".join(program),
            evidence={
    ...
    def _verify(self, task: CurriculumTask, attempt: AttemptResult):
        candidate = self._candidates[task.task_id]
        holdouts = tuple(self._case(x) for x in task.payload["holdouts"])
        ...
        return VerificationResult(
            ...
            reason=(
                ...
                f"unit={decision.unit_passed}/{decision.unit_total};"
...
def build_primitive_self_curriculum(**kwargs) -> PrimitiveSelfCurriculum:

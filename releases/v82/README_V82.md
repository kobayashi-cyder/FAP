# FAP V82 — Bidirectional Visual IR Cognition

V82 adds a shared visual state space so FAP does not generate images directly from language. The mandatory path is:
...
```bash
python -m compileall -q releases/v82/visual_cognition
python -m unittest discover -s releases/v82/visual_cognition/tests -p "test_*.py" -v
```

# FAP V82 — Self-Generated Curriculum

V82 adds a persistent self-generated curriculum controller above the current FAP learning stack.
...
V82 does not persist a raw reasoning trace.
...
`PrimitiveSelfCurriculum` directly connects V82 to the existing V80:
...
Only a candidate that V80 promotes to `active` counts as a V82 success.

This means the first concrete V82 curriculum does not merely mark its own answer correct. It delegates the judgment to the existing bounded sandbox and promotion system.

## Relationship to V81

V81 provides cheap verifier-gated local predictive adaptation. V82 is an upper-level learning-policy layer: it decides which capability to exercise and when to increase difficulty. V81 can remain a local adaptation substrate while V82 controls curriculum selection.

## Scope

V82 implements the autonomous curriculum mechanism and one real verified practice environment. It does not claim arbitrary competence. Expanding beyond Mini-IR requires additional safe task generators and independent verifiers for new reasoning domains.

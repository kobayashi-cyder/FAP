...
class V84SelfCurriculumTests(unittest.TestCase):
    ...
    def test_non_independent_success_does_not_advance_ability(self):
    ...
    def test_evidence_digest_is_bound_to_challenge_content(self):
        ...
        first = CurriculumTask(
            "self:repair:1",
            "repair",
            "repair task at difficulty 0.40",
            0.4,
            payload={"case": 1},
        )
        second = CurriculumTask(
            "self:repair:1",
            "repair",
            "repair task at difficulty 0.55",
            0.55,
            payload={"case": 2},
        )
        p1 = compressor.compress(first, attempt, verification)
        p2 = compressor.compress(second, attempt, verification)
        self.assertNotEqual(
            p1.evidence_digests[0],
            p2.evidence_digests[0],
        )
        store = PatternStore()
        self.assertTrue(store.add(p1))
        self.assertTrue(store.add(p2))

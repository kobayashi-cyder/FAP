...
class PCM16Input:
    ...
    def validate(self):
        ...
        return self
    def rms(self) -> float:
        self.validate(); samples=[int.from_bytes(self.data[i:i+2],"little",signed=True) for i in range(0,len(self.data),2)]
        return math.sqrt(sum(x*x for x in samples)/len(samples))

class AudioInputSkill:
    def __init__(self, stt: Optional[SpeechToTextProvider]=None, speech_rms_threshold: float=250.0): self.stt=stt; self.threshold=float(speech_rms_threshold)
    def inspect(self,audio:PCM16Input)->SkillDecision:
        rms=audio.rms(); return SkillDecision("ok","audio inspected",metadata={"rms":rms,"speech_likely":rms>=self.threshold})

...
class VoiceConversationSkill:
    """Half-duplex voice turn: STT -> text responder -> TTS."""
    def __init__(self, audio_in: AudioInputSkill, audio_out: AudioOutputSkill, responder: Callable[[str], str]):
        self.audio_in=audio_in; self.audio_out=audio_out; self.responder=responder
    ...
    def run_turn(self,audio:PCM16Input,*,voice:str="default")->SkillDecision:
        ...
        if not response: raise ValueError("responder returned empty text")
        tts=self.audio_out.run(TTSRequest(response,voice=voice))
        if tts.status != "ok":
            return SkillDecision(tts.status,tts.reason,metadata={"transcript":transcript,"response_text":response,**tts.metadata})
        return SkillDecision("ok","voice turn completed",artifact=tts.artifact,
                             metadata={"transcript":transcript,"response_text":response,"audio_digest":tts.artifact.digest})

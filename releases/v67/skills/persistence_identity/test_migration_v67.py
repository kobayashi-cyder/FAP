...
def make_v66_fixture(path: str) -> None:
    with sqlite3.connect(path) as con:
        ...
        con.execute(
            ...
            ("failure-1", "cap-a", "digest-a", "family-a", "regression", 1.0),
        )
        con.execute(
            "INSERT INTO releases VALUES(?,?,?,?,?,?,?,?,?)",
            ("release-1", "cap-a", "digest-a", "family-a", "candidate", "a" * 64, 0.9, 0.9, 1.0),
        )
...
class V67MigrationTests(unittest.TestCase):
    def test_migration_preserves_rows_and_scopes_event_ids(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with sqlite3.connect(path) as con:
                ...
                con.execute(
                    "INSERT INTO failures VALUES(?,?,?,?,?,?)",
                    ("failure-1", "cap-b", "digest-b", "family-b", "regression", 2.0),
                )
                ...
                self.assertEqual(con.execute("SELECT COUNT(*) FROM failures").fetchone()[0], 2)
    ...
    def test_holdout_evidence_remains_globally_single_use(self):
        with tempfile.TemporaryDirectory() as td:
            path = str(Path(td) / "state.sqlite3")
            make_v66_fixture(path)
            migrate_v66_identity_schema(path)
            with sqlite3.connect(path) as con:
                with self.assertRaises(sqlite3.IntegrityError):
                    con.execute(
                        "INSERT INTO releases VALUES(?,?,?,?,?,?,?,?,?)",
                        ("release-2", "cap-b", "digest-b", "family-b", "candidate", "a" * 64, 0.9, 0.9, 2.0),
                    )

    def test_restart_is_idempotent_and_keeps_backups(self):
        with tempfile.TemporaryDirectory() as td:
            path = str(Path(td) / "state.sqlite3")
            make_v66_fixture(path)
            migrate_v66_identity_schema(path)
            ...
            self.assertTrue(second["failures_backup"])
    ...
    def test_rollback_restores_v66_global_event_identity(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with sqlite3.connect(path) as con:
                ...
                self.assertEqual(con.execute("SELECT COUNT(*) FROM failures").fetchone()[0], 1)

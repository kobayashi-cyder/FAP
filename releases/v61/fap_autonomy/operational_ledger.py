...
class OperationalLedger:
    ...
    It stores only verified outcomes accepted by V59OutcomeAdapter. A deterministic
    audit bucket is reserved from diagnosis so repeated runs cannot leak those rows
    into capability selection.
    """
    ...
    def _result_to_payload(r: BenchmarkResult) -> Dict:
        return {
            ...
            "case_metadata": r.case.metadata,
            "success": r.success,
            "actual": r.actual,
            "error": r.error,
            "latency_ms": r.latency_ms,
            "teacher_used": r.teacher_used,
            "verified": r.verified,
    ...
    def _payload_to_result(p: Dict) -> BenchmarkResult:
        case = BenchmarkCase(
            case_id=str(p["id"]), domain=str(p.get("domain", "unknown")),
            task=str(p.get("task", "")), expected=p.get("expected"),
            metadata=dict(p.get("case_metadata", {})),
        )
        return BenchmarkResult(
            case=case, success=bool(p.get("success", False)), actual=p.get("actual"),
    ...
    def ingest_rows(self, rows: Iterable[Dict]) -> Dict[str, int]:
        ...
        with self._connect() as cx:
            for r in verified:
                ...
                cur = cx.execute(
                    "INSERT OR IGNORE INTO verified_events(turn_id,bucket,payload_json) VALUES(?,?,?)",

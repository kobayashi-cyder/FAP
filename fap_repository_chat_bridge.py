...
class RepositoryChatBridge:
    ...
    def build(
        ...
        *,
        ...
        max_history_rows: int = 1_000,
    ) -> ChatCodingRequest:
        max_turns = int(max_turns)
        max_goal_chars = int(max_goal_chars)
        max_file_hints = int(max_file_hints)
        max_history_rows = int(max_history_rows)
        if not 0 <= max_turns <= 100:
            raise ValueError("max_turns must be in [0, 100]")
        if not 1 <= max_goal_chars <= 100_000:
            raise ValueError("max_goal_chars must be in [1, 100000]")
        if not 0 <= max_file_hints <= 128:
            raise ValueError("max_file_hints must be in [0, 128]")
        if not 1 <= max_history_rows <= 10_000:
            raise ValueError("max_history_rows must be in [1, 10000]")
        ...
        _strict_utf8(goal, "current_text")
        ...
        rows: list[Mapping[str, object] | str] = []
        for index, row in enumerate(history):
            if index >= max_history_rows:
                raise ValueError("history exceeds max_history_rows")
            rows.append(row)
        considered = rows[-max_turns:] if max_turns else []
        texts = tuple(_message_text(row) for row in considered)
        for text in texts:
            _strict_utf8(text, "history")
        digest_payload = json.dumps(texts, ensure_ascii=False, separators=(",", ":"))
        history_digest = sha256(digest_payload.encode("utf-8", errors="strict")).hexdigest()

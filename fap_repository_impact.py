...
class RepositoryImpactAnalyzer:
    """Read-only reverse dependency analysis for repository coding changes.

    The indexer already records Python import edges as source -> target. Coding
    verification needs the inverse question: which files and tests may be
    affected when a target changes? This analyzer performs a bounded reverse
    breadth-first walk without importing or executing repository code.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._known = {row.path for row in self.index.files}
    ...
    def analyze(self, paths: Iterable[str]) -> RepositoryImpactReport:
        ...
        missing = tuple(path for path in seeds if path not in self._known)
        if missing:

...
class RepositoryImpactAnalyzer:
    ...
    def analyze(self, paths: Iterable[str]) -> RepositoryImpactReport:
        ...
        impacted = tuple(
            ImpactedPath(path=path, distance=distances[path], reason=reasons[path])
            for path in sorted(distances, key=lambda value: (distances[value], value))
        )
        tests = tuple(
            item.path for item in impacted if _is_python_test_path(item.path)
        )

...
class V8781CodingConversationGatewayTests(unittest.TestCase):
    ...
    def test_response_series_redundancy_executes_not_just_plans(self) -> None:
        ...
        self.assertEqual(response.get("native_parity_revision"), "1.0.01-cpp-native-r008")
        self.assertIn("factual", response.get("safe_specialists", []))
        self.assertIn("derivation", response.get("safe_specialists", []))
        self.assertEqual(len(response.get("safe_specialists", [])), 16)
        ...
        self.assertIn("multi_step_math", response.get("safe_specialists", []))
        self.assertIn("python_repair", response.get("safe_specialists", []))
        self.assertIn("causal_graph", response.get("safe_specialists", []))
        self.assertIn("counterexample_search", response.get("safe_specialists", []))
        self.assertIn("longform_contradiction", response.get("safe_specialists", []))

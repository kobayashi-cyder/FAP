...
class DynamicSparseRoutingTests(unittest.TestCase):
    ...
    def test_explicit_path_budget_caps_high_risk_route_depth(self):
        router = DynamicSparseRouter(
            routes(),
            config=DynamicSparseConfig(
                warmup_steps=0,
        ...
        uncapped = router.select(context, top_k=4)
        capped = router.select(
            context,
            top_k=1,
            max_path_routes=1,
        )
        self.assertGreaterEqual(len(uncapped.primary_path.route_ids), 2)
        self.assertEqual(len(capped.primary_path.route_ids), 1)

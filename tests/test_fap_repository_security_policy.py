...
class RepositorySecurityPolicyTests(unittest.TestCase):
    ...
    def _edit(self, plan) -> FileEdit:
        row = next(x for x in plan.files if x.path == "calc.py")
        return FileEdit("calc.py", "modify", row.before_sha256, "x = 2\n")
    ...
    def test_path_qualified_allowed_name_must_match_known_path(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            plan = self._plan(root)
            command = VerificationCommand(
    ...
    def test_phase_timeout_and_control_characters_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            plan = self._plan(root)
            command = VerificationCommand(
            ...
            codes = {x.code for x in report.findings}
            self.assertFalse(report.allowed)
            self.assertIn("unsupported_command_phase", codes)
            self.assertIn("invalid_command_timeout", codes)
            self.assertIn("invalid_argv_value", codes)

    def test_non_ready_or_write_enabled_plan_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            plan = self._plan(root)
            report = RepositorySecurityPolicy().audit(
                plan=replace(plan, status="stale_context", write_enabled=True)
            )
            self.assertFalse(report.allowed)
            codes = {x.code for x in report.findings}
            self.assertIn("plan_not_ready", codes)

...
class RepositoryImpactAnalyzerTests(unittest.TestCase):
    ...
    def test_reverse_dependency_walk_finds_indirect_test(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            self._fixture(root)
            report = RepositoryImpactAnalyzer(root, max_depth=2).analyze(
                ("core.py",)
            )
    ...
    def test_depth_bound_stops_second_hop(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            self._fixture(root)
            report = RepositoryImpactAnalyzer(root, max_depth=1).analyze(
                ("core.py",)
            )
    ...
    def test_unknown_seed_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaisesRegex(ValueError, "unknown repository paths"):
                RepositoryImpactAnalyzer(root).analyze(("missing.py",))

    def test_seed_count_is_bounded(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            self._fixture(root)
            with self.assertRaisesRegex(ValueError, "seed path count"):
                RepositoryImpactAnalyzer(
                    root,
                    max_impacted=1,
                ).analyze(("core.py", "service.py"))

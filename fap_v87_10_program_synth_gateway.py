...
class SessionMemory:
    ...
    def clear(self, sid: str) -> None:
        """Delete persisted and cached raw history for one session."""
        key = self._key(sid)
    ...
    def verify(self, intent: Intent, result: dict) -> tuple[str, str]:
        ...
        if result.get("image_orchestrated"):
            ...
            if result.get("local_raster") and result.get("structural_verified"):
                if result.get("quality_met"):
                    return "OK", "FAP内蔵ラスタ生成器が要求された簡易図解をPNGへ描画し、成果物構造を検証しています。"
                return "PARTIAL", "PNGとしての生成と構造検証には成功しましたが、これは簡易図解ドラフトであり、通常の画像生成としての品質基準は満たしていません。"
...
class FAPV8710:
    @classmethod
    def _runtime_generation(cls) -> tuple[int, int]:
        """Return the effective mainline generation of the concrete facade.

        Shared implementation modules may contain newer organs, but an older
        facade must not accidentally activate capabilities introduced later.
        """
        module = sys.modules.get(cls.__module__)
        raw = str(getattr(module, "VERSION", VERSION))
    ...
    @classmethod
    def _has_generation(cls, minor: int) -> bool:
        major, current_minor = cls._runtime_generation()
        return major > 87 or (major == 87 and current_minor >= int(minor))

    @classmethod
    def _inquiry_profile(cls) -> dict[str, Any]:
        _, minor = cls._runtime_generation()
        if minor < 46:
            return {
                "default_target": 5,
                "max_target": 32,
    ...
    def __init__(self):
        ...
        self.inquiry = InquiryEngine(ROOT, **self._inquiry_profile())
    ...
    def route(self, intent: Intent, text: str, history: list[dict]) -> dict:
        ...
        scientific_model = (
            self.scientific_model.run(text, history)
            if self._has_generation(49)
            else None
        )
        ...
        cycle = (
            self.research_cycle.run(text, history)
            if self._has_generation(54)
            else None
        )
        ...
        frontier = (
            self.research_frontier.run(text, history)
            if self._has_generation(53)
            else None
        )
        ...
        hypothesis = (
            self.hypothesis.run(text, history)
            if self._has_generation(52)
            else None
        )
        ...
        inquiry = (
            self.inquiry.run(text, history)
            if self._has_generation(45)
            else None
        )
        ...
        reflective = (
            self.reflective.run(text, history)
            if self._has_generation(43)
            else None
        )
